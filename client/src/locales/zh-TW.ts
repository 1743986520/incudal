...
4. 工單建立後可在詳情查看工單 ID、狀態、分類、優先級、宿主機與實例；已關閉的工單不能回覆。不要在內容或附件中貼 SSH 私鑰、密碼或 Agent 密鑰。`
    ...
    admin: {
        ...
        hosts: {
            ...
            forcePanelDelete: '強制從面板刪除',
            forcePanelDeleteConfirm: '節點可能無法通訊。強制刪除只會清理面板側實例記錄，不會嘗試停止或刪除主機上的實例；符合條件的付費託管實例仍會退款並記錄餘額變動。確定刪除這 {count} 個實例嗎？',
            forcePanelDeleteHint: '如果節點已完全離線，請使用「強制從面板刪除」。此模式不會連接節點或操作主機上的實例，只清理面板側實例記錄，並按規則執行退款和記錄餘額變動。',
        ...
        officialCoupons: {
            ...
            renewalModeLimitedTimes: '限定折價 {count} 次',
            discountedChargeLimit: '折價次數（含首次購買）',
            discountedChargeLimitHint: '例：填 9 表示含首次購買共折價 9 次，第 10 次起按原價續費。',
            validity: '有效期',
            alwaysValid: '長期有效',
            noStart: '立即生效',
            noEnd: '無結束時間',
            ...
            validation: {
                ...
                chargeLimitRequired: '限定折價次數模式下必須填寫不小於 1 的整數',
            },
        },
