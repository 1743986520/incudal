...
## 2026-09-01 — PPS 突发计数修正

- 修正节点安装脚本的 PPS token bucket 配置：默认突发从 5,000/2,500 包降为 1 包，避免 20,000/10,000 PPS 规则在短时间内额外放行大量数据包。
- 保留单包调度余量；PPS 规则仍按来源 MAC 及单一目的 IP 分别计数。
